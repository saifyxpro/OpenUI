...
export function getElementAtPoint(x: number, y: number) {
  ...
  const refElement =
    (elementsBelowAnnotation.find(
      (element) =>
        !element.closest('svg') &&
        !element.closest('OPENUI-TOOLBAR') &&
...
const isReactInternalProperty = (key: string): boolean =>
...
export const getSelectedElementInfo = (
  ...
  const rawOwnProperties = Object.getOwnPropertyNames(element)
    .filter((prop) => !excludedProperties.has(prop) && !isReactInternalProperty(prop))
    .reduce(
      (acc, prop) => {
        try {
  ...
  const isParentCall = (callDepth ?? 0) > 0;
  ...
  return {
    ...
    textContent: isParentCall ? '' : truncateString(element.textContent || '', 512),
    ownProperties: isParentCall ? {} : truncateOwnProperties(rawOwnProperties),
    boundingClientRect: isParentCall
      ? { top: 0, left: 0, height: 0, width: 0 }
      : { top: boundingRect.top, left: boundingRect.left, height: boundingRect.height, width: boundingRect.width },
    parent:
      element.parentElement && (callDepth ?? 0) < 1
        ? getSelectedElementInfo(element.parentElement, (callDepth ?? 0) + 1)
        : null,
    pluginInfo: isParentCall ? [] : truncatePluginInfo([]),
  };
};
