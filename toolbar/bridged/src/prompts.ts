...
import { extractSkillMentions, stripSkillMentions } from './config/skill-tags';
...
export function createPrompt(
  ...
  const skillMentions = extractSkillMentions(userPrompt);
  const cleanedUserPrompt = stripSkillMentions(userPrompt);

  const skillsSection =
    skillMentions.length > 0
      ? `\n<skills>\n${skillMentions.map((s) => `  <skill>${s}</skill>`).join('\n')}\n</skills>`
      : '';
...
<selected_elements>
${elementContext}</selected_elements>${skillsSection}${pluginContext}`.trim();
}
