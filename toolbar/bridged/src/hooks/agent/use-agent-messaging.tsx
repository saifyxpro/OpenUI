...
export const AgentMessagingProvider = ({
  ...
  const handleUserMessage = (message: UserMessage) => {
    console.log('[OpenUI:agentMessaging] sendUserMessage called', { messageId: message.id, agentConnected: !!agent, agentPort: agent?.port });
    agent?.agent.messaging.sendUserMessage.mutate(message);
    console.log('[OpenUI:agentMessaging] mutate fired');
  };
