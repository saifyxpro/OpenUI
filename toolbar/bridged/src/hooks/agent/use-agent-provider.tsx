...
function persistSelectedAgent(agent: AgentInfo): void {
  try {
    ...
    localStorage.setItem('openui_toolbar_selected_agent', uniqueKey);
...
function getPersistedAgentKey(): string | null {
  try {
    return localStorage.getItem('openui_toolbar_selected_agent');
  } catch (error) {
...
function persistClipboardMode(): void {
  try {
    localStorage.setItem('openui_toolbar_selected_agent', 'clipboard');
...
function isClipboardModePersisted(): boolean {
  try {
    const persisted = localStorage.getItem('openui_toolbar_selected_agent');
...
function clearPersistedSelection(): void {
  try {
    localStorage.removeItem('openui_toolbar_selected_agent');
...
/**
 * Checks if an agent is available on the specified port by calling the /openui/info endpoint.
...
async function checkAgentOnPort(
  ...
  try {
    ...
    if (response.ok) {
      const info: OpenUIInfo = await response.json();
      // Validate that it's a valid OpenUIInfo object
      if (
        info &&
...
export function AgentProvider({ children }: { children?: ReactNode }) {
  ...
  const usesOpenUIAgent = config?.usesOpenUIAgent || false;
        ...
        const { client, wsClient } = createWebSocketClient(
          ...
          () => {
            ...
            if (!isManualSelectionRef.current && !isClipboardModePersisted()) {
              ...
              console.info(
                `[openui] Searching for available agents after connection loss...`,
              );
              scanAgents();
            } else {
              console.debug(
  ...
  useEffect(() => {
    ...
    if (!finishedInitialScan) {
      ...
      if (usesOpenUIAgent) {
        console.debug(
          '[AgentProvider] Config indicates OpenUI agent usage, connecting to app port...',
        );
      ...
      console.debug(
        '[AgentProvider] No OpenUI agent configured, scanning for regular agents...',
      );
      scanAgents();
    }
