...
const fakeDiagCollection =
  vscode.languages.createDiagnosticCollection('openui');

// Create output channel for openui
const outputChannel = vscode.window.createOutputChannel('openui');

// Handler for the setupToolbar command
async function removeOldToolbarHandler() {
  await removeOldToolbar();
  await vscode.window.showInformationMessage(
    "The agent has been triggered to remove the old integration of openui. Please follow the agent's instructions in the chat panel.",
    'OK',
  );
}
...
async function initializeIDEAgent() {
  ...
  AgentSelectorService.getInstance().updateStatusbarText('Connected');
  ...
  try {
    ...
    if (ide === 'UNKNOWN') {
      vscode.window.showInformationMessage(
        'openui does not work for your current IDE.',
      );
      return;
    }
    ...
    const configChangeListener = vscode.workspace.onDidChangeConfiguration(
      async (e) => {
        if (e.affectsConfiguration('openui.telemetry.enabled')) {
          const config = vscode.workspace.getConfiguration('openui');
