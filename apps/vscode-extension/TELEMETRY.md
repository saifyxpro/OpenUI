...
The openui code editor extension collects pseudonymized usage data to help improve our products and services. We take your privacy seriously and are committed to transparency about our data collection practices.
...
- Toolbar update recommendation events (`show_toolbar_update_notification`, `toolbar_update_notification_auto_update`, `toolbar_update_notification_ignored`, `toolbar_update_notification_dismissed`)
  - When users get recommended updating the openui toolbar on loading of workspaces
...
- `openui_agent_prompt_triggered`: Triggered when the openui Agent is invoked
  - Includes: agent type ('openui'), whether user included a message, message ID, current URL, number of selected elements, number of prompt snippets
- `openui_agent_tool_call_requested`: Triggered when the openui Agent requests a tool execution
  - Includes: agent type ('openui'), tool name, whether it's a client-side tool, whether it's a browser runtime tool
...
- `toolbar_auto_update_prompt_sent`: Triggered then the prompt for updating the toolbar was actually sent
  - No additional properties collected
- `show_toolbar_integration_notification`: Triggered when the user is shown a recommendation to integrate openui into their app
  - No additional properties collected
...
- `toolbar_setup_failed`: Triggered when the toolbar setup process fails
  - Includes: Error message (scrubbed of PII)
- `openui_agent_auth_refresh_failed`: Triggered when automatic token refresh fails in the openui Agent
...
To disable telemetry just for openui:
1. Open VS Code Settings (Ctrl+,/Cmd+,)
2. Search for "openui telemetry"
3. Uncheck "openui: Telemetry Enabled"
...
- [openui Privacy Policy](https://openui.dev/privacy)

## Questions or Concerns?

If you have questions about our telemetry practices, please:
1. File an issue on our [GitHub repository](https://github.com/saifyxpro/openui/issues)
2. Contact us through our [website](https://openui.dev/contact)