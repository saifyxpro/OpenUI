...
const DEPENDENCY_MATCHERS: DependencyMatcher[] = [
  {
    plugin: '@openui-dev-plugins/react',
    dependencies: {
      react: '*',
      'react-dom': '*',
      next: '*',
  ...
  {
    plugin: '@openui-dev-plugins/angular',
    dependencies: {
      '@angular/core': '*',
      '@angular/common': '*',
...
function getBundledPluginPath(pluginName: string): string {
  const simpleName = pluginName.replace('@openui-dev-plugins/', '');
...
export function generatePluginImportMapEntries(
  ...
  availablePlugins.forEach((plugin, index) => {
    ...
    if (plugin.url) {
      ...
      entries[entryName] =
        `/openui-toolbar-app/plugins/${pluginName}/index.js`;
    }
  });
