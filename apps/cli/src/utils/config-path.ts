...
// Use 'openui-dev' in dev mode to keep dev and production configs separate
const appName = isDevMode() ? 'openui-dev' : 'openui';
...
export const openui: OpenUIConfig = {
...
/**
 * Gets a file path within the openui config directory
 */
export function getConfigPath(filename: string): string {
  return path.join(openui.configDir, filename);
}

/**
 * Gets a file path within the openui data directory
 */
export function getDataPath(filename: string): string {
  return path.join(openui.dataDir, filename);
}
...
export async function writeConfigFile<T>(
  ...
  const filePath = getConfigPath(filename);
  await ensureDir(openui.configDir);
  try {
...
export async function writeDataFile<T>(
  ...
  const filePath = getDataPath(filename);
  await ensureDir(openui.dataDir);
  try {
    const content = JSON.stringify(data, null, 2);
    await fs.writeFile(filePath, content, { mode: 0o600 });
  } catch (error) {
