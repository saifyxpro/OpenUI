# OpenUI CLI Architecture

## Overview
The OpenUI CLI is a Node.js application that provides a local development server with proxy capabilities, authentication, and plugin support.
...
- **Command Wrapping**: Executes external commands via CommandExecutor while running OpenUI server concurrently
...
- Sources (in priority order):
  ...
  3. Configuration file (openui.config.json)
  4. Default values
...
- **Agent Loader**: Initializes OpenUI agent with WebSocket support (disabled in bridge mode)
...
### Main Command
```
openui [options] [-- <command>]
```

Starts the OpenUI development server with toolbar integration.

**Options:**
- `-p, --port <port>`: Port for the OpenUI server (default: 3000)
- `-a, --app-port <app-port>`: Port of your application that OpenUI will proxy and enhance with toolbar
...
```
openui [options] -- <command> [args...]
```

Executes external commands while running OpenUI server concurrently in the background. Uses double-dash delimiter pattern similar to dotenv-cli.

**Examples:**
- `openui -- npm run dev`
- `openui -p 3100 -- tsx src/build.ts --watch`
- `openui -- node script.js --flag value`
- `openui -b -- pnpm build` (bridge mode with command wrapping)
...
- Runs OpenUI server concurrently in background
- Minimal configuration required (silent mode enabled by default)
- Works with both standard and bridge modes

### Auth Command
```
openui auth <subcommand>
```

Manage authentication for OpenUI CLI.

**Subcommands:**
- `login`: Authenticate with OpenUI
- `logout`: Clear stored authentication tokens
- `status`: Check authentication status

### Telemetry Command
```
openui telemetry <subcommand>
```

Manage telemetry settings for OpenUI CLI. Telemetry tracks usage metrics and failure events without capturing request content.
...
- Starts full OpenUI server with agent capabilities
...
- Opens browser automatically to the OpenUI interface
...
Executes external commands while running OpenUI server concurrently.
- Uses double-dash delimiter pattern (similar to dotenv-cli)
- Runs OpenUI server in background while executing user command
...
- Examples: `openui -- npm run dev`, `openui -p 3100 -- tsx build.ts`
...
- **Dev Mode Detection**: Automatically uses separate config directory in development
  - Production: Uses `openui` app name for env-paths
  - Development: Uses `openui-dev` app name for env-paths
  - Detection methods:
