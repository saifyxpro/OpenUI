...
describe('Config Error Handling Integration Tests', () => {
  ...
  it('should gracefully handle invalid JSON in openui.json', async () => {
    // Create invalid JSON file
    await fs.writeFile(
      path.join(testWorkspace, 'openui.json'),
    ...
    await new Promise<void>((resolve) => {
      child.on('exit', (code) => {
        ...
        expect(code).toBe(1);
        expect(combinedOutput).toContain('Failed to parse openui.json');
        expect(combinedOutput).toContain(
          'Please fix the errors in openui.json and try again',
        );
        resolve();
      });
    });
  ...
  it('should gracefully handle invalid port number', async () => {
    ...
    await new Promise<void>((resolve) => {
      child.on('exit', (code) => {
        ...
        expect(code).toBe(1);
        expect(combinedOutput).toContain(
          'Invalid configuration in openui.json',
        );
        expect(combinedOutput).toContain(
