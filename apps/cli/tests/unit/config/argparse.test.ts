...
describe('argparse', () => {
  ...
  describe('command wrapping with double-dash', () => {
    ...
    it('should correctly parse openui flags before double-dash and ignore command args after', async () => {
      ...
      expect(args.port).toBe(4000); // OpenUI port, not the one in wrapped command
      expect(args.verbose).toBe(true);
    });
  });
});
